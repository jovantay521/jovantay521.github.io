...
def displayDriveInfo(data):
    ...
    return render_template('route_planner.html', route1Steps=routeSteps[0], route2Steps=routeSteps[1], route3Steps=routeSteps[2]) #rendering an empty array will not have any effect
    ...
    for itenary in data: #each itenary is an object containing 1 route data
        ...
        for routeData in route: #for each step of the route
            if routeData["mode"]=="WALK":
                ...
                continue
        counter+=1
    return render_template("route_planner.html", route1Steps=routeSteps[0], route2Steps=routeSteps[1], route3Steps=routeSteps[2])
